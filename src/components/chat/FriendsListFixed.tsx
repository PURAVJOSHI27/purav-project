...
export const FriendsList: React.FC<FriendsListProps> = ({ user, onSignOut, onFriendSelect }) => {
  ...
  const handleImageError = (e: React.SyntheticEvent<HTMLImageElement>) => {
    console.log('Photo loading error for user:', user.displayName);
      ...
      <div className="bg-white/90 backdrop-blur-sm border-b border-primary-200 p-4">
        ...
        <div className="flex items-center space-x-3 p-3 bg-primary-50 rounded-xl">
          <div className="relative">
            ...
            <div 
              className={`fallback-avatar w-12 h-12 bg-primary-500 rounded-full flex items-center justify-center text-white font-bold ${user.photoURL ? 'hidden' : 'flex'}`}
            ...
            >
              ...
              <div className="relative">
                <img
                  src={friend.avatar}
                  alt={friend.name}
                  className="w-14 h-14 rounded-full border-2 border-white shadow-sm group-hover:border-primary-200 transition-colors object-cover"
                />
                {friend.isOnline && (
